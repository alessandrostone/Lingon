	...
	if ([[LINDefaults valueForKey:@"CheckForUpdatesInterval"] intValue] != LINCheckForUpdatesNever) {
		...
		if ([LINDefaults valueForKey:@"LastCheckForUpdateDate"] == nil) {
			...
			if ([[LINDefaults valueForKey:@"CheckForUpdatesInterval"] intValue] == LINCheckForUpdatesDaily) {
				if ([[lastCheckDate dateByAddingTimeInterval:(60 * 60 * 24)] compare:[NSDate date]] == NSOrderedAscending) {
					checkForUpdates = YES;
				}
			} 
			else if ([[LINDefaults valueForKey:@"CheckForUpdatesInterval"] intValue] == LINCheckForUpdatesWeekly) {
				if ([[lastCheckDate dateByAddingTimeInterval:(60 * 60 * 24 * 7)] compare:[NSDate date]] == NSOrderedAscending) {
					checkForUpdates = YES;
				}
			} 
			else if ([[LINDefaults valueForKey:@"CheckForUpdatesInterval"] intValue] == LINCheckForUpdatesMonthly) {
				if ([[lastCheckDate dateByAddingTimeInterval:(60 * 60 * 24 * 30)] compare:[NSDate date]] == NSOrderedAscending) {
					checkForUpdates = YES;
				}
			}
		}
...
{	
	...
	SCNetworkReachabilityRef target = SCNetworkReachabilityCreateWithName(NULL, "lingon.sourceforge.net");

	BOOL success = SCNetworkReachabilityGetFlags(target, &flags); 
	BOOL connected = success && (flags & kSCNetworkFlagsReachable) && !(flags & kSCNetworkFlagsConnectionRequired); 
